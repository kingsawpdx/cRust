...
pub fn draw_ui(frame: &mut Frame, app: &mut App) {
    ...
    let info_items = [
        (
            ...
            app.current_player.available_sandwiches,
        ),
        (
            "Sandwiches per Second",
            app.current_player.sandwiches_per_second,
        ),
        (
    ...
    let automation_info = vec![
        ...
        ("cRust-way", app.current_player.crustway, 5, "🏪"),
        ("cRust-azon", app.current_player.crustazon, 10, "📦"),
    ];
    ...
    for (name, owned, per_item, symbol) in automation_info {
        ...
        automation_text.push(Line::from(format!(
            "  Each generates: {} sandwich{}/sec",
            per_item,
            if per_item != 1 { "es" } else { "" }
        )));
        automation_text.push(Line::from(format!(
            "  Total output: {} sandwich{}/sec",
            total_production,
    ...
    let progress_percent =
        (app.current_player.total_sandwiches_made as f64 / u16::MAX as f64) * 100.0;
